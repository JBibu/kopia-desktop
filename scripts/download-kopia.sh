...
download() {
    ...
    if [ "$success" = true ]; then
        ...
        echo "   ⚠️  Download failed after $max_retries attempts"
        FAILED_DOWNLOADS=$((FAILED_DOWNLOADS + 1))
        return 1
    fi
    echo ""
...
if [ -n "$PLATFORM_ONLY" ]; then
    ...
    download "Linux x64" "kopia-${VERSION}-linux-x64.tar.gz" "kopia-linux-x64" || true
    download "Linux ARM64" "kopia-${VERSION}-linux-arm64.tar.gz" "kopia-linux-arm64" || true
    download "macOS x64" "kopia-${VERSION}-macOS-x64.tar.gz" "kopia-darwin-x64" || true
    download "macOS ARM64" "kopia-${VERSION}-macOS-arm64.tar.gz" "kopia-darwin-arm64" || true
    download "Windows x64" "kopia-${VERSION}-windows-x64.zip" "kopia-windows-x64.exe" || true
fi

echo ""
if [ $FAILED_DOWNLOADS -gt 0 ]; then
    echo "⚠️  Warning: $FAILED_DOWNLOADS download(s) failed"
    if [ -n "$PLATFORM_ONLY" ]; then
        echo "❌ Failed to download required platform binary"
        exit 1
    else
        echo "⚠️  Some platform binaries are missing (this is OK for development)"
    fi
else
    echo "✅ Done!"
fi
