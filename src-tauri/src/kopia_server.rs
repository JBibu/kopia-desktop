...
impl KopiaServer {
    ...
    /// Get the repository ID this server belongs to
    #[allow(dead_code)]
    pub fn get_repo_id(&self) -> Option<&str> {
        self.repo_id.as_deref()
    }

    /// Get server info if available
    #[allow(dead_code)]
    pub fn get_info(&self) -> Option<KopiaServerInfo> {
        self.info.clone()
    }
    ...
    /// - Random password generated by Kopia (not visible in process list)
    /// - Graceful shutdown via stdin close
    pub fn start_with_config(&mut self, config_dir: &str, repo_id: &str) -> Result<KopiaServerInfo> {
        if self.is_running() {
