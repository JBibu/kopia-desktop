...
test.describe('Repository Setup - Navigation', () => {
  ...
  test('can select filesystem storage provider', async ({ page }) => {
    ...
    if (await filesystemOption.isVisible()) {
      ...
      // Should show filesystem configuration fields
      await expect(page.getByText(/path|directory/i)).toBeVisible();
    }
  });
    ...
    if (await filesystemOption.isVisible()) {
      ...
      if (await pathInput.isVisible()) {
        ...
        if (await nextButton.isVisible()) {
          ...
          // Should proceed to next step (password)
...
test.describe('Repository Page - Connected State', () => {
  ...
  test('displays repository information', async ({ page }) => {
    ...
    // Look for repository status information
    const statusElements = [/connected|disconnected/i, /storage|provider/i, /description/i];
