...
import { test, expect, navigateToPage, waitForNavigation } from './fixtures';
...
test.describe('Backup Profiles - Navigation', () => {
  ...
  test('can navigate to Backups page', async ({ page }) => {
    ...
    await waitForNavigation(page, '/snapshots');

    await expect(page.getByRole('heading', { name: /backups|snapshots/i })).toBeVisible();
  });
...
test.describe('Backup Profiles - Profile List', () => {
  ...
  test('profile cards show status badges', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const statusBadge = firstCard.locator('[data-testid="profile-status"]');
