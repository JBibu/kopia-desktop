...
describe('Profile Workflows (Integration)', () => {
  ...
  describe('Complete backup profile lifecycle', () => {
    it('creates, edits, uses, and deletes a profile', async () => {
      ...
      // 2. User adds more directories to the profile
      store.addDirectory(profile.id, '/home/user/pictures');
      store.addDirectory(profile.id, '/home/user/videos');

      const updated = useProfilesStore.getState().profiles[0];
      expect(updated.directories).toHaveLength(3);
      expect(updated.directories).toEqual([
        '/home/user/documents',
        '/home/user/pictures',
        '/home/user/videos',
      ]);
      ...
      store.removeDirectory(profile.id, '/home/user/videos');
  ...
  describe('Directory management edge cases', () => {
    it('handles adding duplicate directories gracefully', () => {
      const store = useProfilesStore.getState();
      const profile = store.createProfile({
        name: 'Test',
      ...
      store.addDirectory(profile.id, '/home/user/docs');

      const updated = useProfilesStore.getState().profiles[0];
      // Should have duplicates (store doesn't prevent this - UI should)
      expect(updated.directories).toContain('/home/user/docs');
    });

    it('handles removing non-existent directory', () => {
      const store = useProfilesStore.getState();
      const profile = store.createProfile({
        name: 'Test',
        directories: ['/home/user/docs'],
      ...
      const initialLength = useProfilesStore.getState().profiles[0].directories.length;

      store.removeDirectory(profile.id, '/non/existent/path');

      const afterLength = useProfilesStore.getState().profiles[0].directories.length;
    ...
    it('handles empty directory list', () => {
      ...
      // Remove the only directory
      store.removeDirectory(profile.id, '/home/user/docs');

      const updated = useProfilesStore.getState().profiles[0];
      expect(updated.directories).toHaveLength(0);
      // Profile should still exist, just with no directories
      expect(useProfilesStore.getState().profiles).toHaveLength(1);
    });
  });
  ...
  describe('Profile state consistency', () => {
    it('maintains profile isolation when operating on one profile', () => {
      ...
      store.addDirectory(profile1.id, '/dir1a');
      store.addDirectory(profile1.id, '/dir1b');
      store.toggleProfile(profile1.id);
      store.togglePin(profile1.id);
      store.updateProfile(profile1.id, { name: 'Modified Profile 1' });
